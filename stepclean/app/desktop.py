...
from stepclean.prediction.hotspots import DEFAULT_HOTSPOT_MODEL_PATH, HotspotPrediction, predict_step_hotspots
...
MODEL_DISPLAY_NAME = "LightGBM stayed-dirty classifier"
MODEL_CV_ACCURACY = 0.8977
MODEL_HOLDOUT_ACCURACY = 0.8866
MODEL_DIRTY_PROBABILITY_THRESHOLD = 0.55
...
def view_step_file(filename: str | Path) -> None:
    ...
    model_status = tk.StringVar(value=_model_status_text())
    water_force_var = tk.StringVar(value=f"{REFERENCE_WATER_FORCE:.3f}")
    model_threshold_var = tk.StringVar(value=f"{MODEL_DIRTY_PROBABILITY_THRESHOLD:.3f}")
    ...
    def load_step_file(new_path: Path) -> None:
        ...
        model_status.set(_model_status_text())
    ...
    def selected_model_threshold() -> float:
        threshold = _parse_probability_threshold(model_threshold_var.get())
        if threshold is None:
            raise ValueError("enter a dirty probability threshold from 0.000 to 1.000")
        return threshold
    ...
    def show_model_vs_retained_particles() -> None:
        cleaning_view_active["value"] = False
        try:
            model_threshold = selected_model_threshold()
        ...
        model_status.set("Classifier: predicting dirty faces and comparing with retained particles...")
        ...
        selected_predictions = _selected_learned_hotspots(predictions, threshold=model_threshold)
        retained_scores = _face_retained_particle_scores(result)
        retained_faces = _selected_retained_particle_faces(retained_scores)
        accepted_retained_faces = _accepted_retained_particle_faces(retained_scores)
        retained_face_set = set(retained_faces)
        ...
        max_model_score = max((prediction.dirty_probability for prediction in predictions), default=0.0)
        ...
        draw_reference_axes()
        model_status.set(
            f"{MODEL_DISPLAY_NAME}: pink={len(selected_predictions)} dirty faces; "
            f"rank colors=top retained particles at force {REFERENCE_WATER_FORCE:.3f} "
        ...
        status.set(
            _status_text(
                ...
                (
                    ...
                    f"rank colors={len(retained_faces)} top retained faces at force={REFERENCE_WATER_FORCE:.3f}, "
                    f"accepted dirty={len(accepted_retained_faces)} faces, "
                    f"dirs={_direction_summary(reference_cleaning_parameters())}, threshold={model_threshold:.3f}, "
                    f"accepted overlap={overlap_count}, "
                    f"order={_retained_rank_summary(retained_faces, retained_scores)}"
                ),
            )
        )
    ...
    def show_learned_hotspots() -> None:
        ...
        model_status.set("Classifier: extracting features and predicting dirty probabilities...")
        status.set(
            _status_text(
                path,
                edge_groups,
        ...
        if not predictions:
            model_status.set("Classifier: no mesh faces were available for prediction")
            status.set(_status_text(path, edge_groups, feature_summary, "no model hotspots"))
            return

        selected = _selected_learned_hotspots(predictions, threshold=model_threshold)
        max_prediction = max(prediction.dirty_probability for prediction in predictions)
        ...
        model_status.set(f"{MODEL_DISPLAY_NAME}: {summary}")
        status.set(
            _status_text(
                path,
                edge_groups,
                ...
                (
                    ...
                    f"force={parameters.water_force:.3f}, threshold={model_threshold:.3f}, "
                    f"max dirty probability={max_prediction:.6f}"
                ),
            )
        )
        display.Repaint()
    ...
    tk.Button(toolbar, text="Predict Dirty Faces", command=show_learned_hotspots).pack(side=tk.LEFT, padx=(0, 12))
    tk.Button(toolbar, text="Run Cleaning Simulation", command=show_cleaning_result).pack(side=tk.LEFT, padx=(0, 12))
    tk.Button(toolbar, text="Compare Model vs Simulation", command=show_model_vs_retained_particles).pack(
        side=tk.LEFT,
        padx=(0, 18),
    )
    ...
    tk.Label(toolbar, text="Range 0.000-1.000").pack(side=tk.LEFT, padx=(0, 12))
    tk.Label(toolbar, text="Dirty Threshold").pack(side=tk.LEFT, padx=(0, 6))
    tk.Entry(
        toolbar,
        width=7,
        textvariable=model_threshold_var,
    ).pack(side=tk.LEFT, padx=(0, 12))
    tk.Label(
        toolbar,
        text=f"{MODEL_DISPLAY_NAME}: CV acc {MODEL_CV_ACCURACY:.4f}",
    ).pack(side=tk.LEFT, padx=(0, 12))
...
def _accepted_retained_particle_faces(face_scores: dict[int, float]) -> list[int]:
    return sorted(face_scores, key=lambda face_id: (-face_scores[face_id], face_id))
...
def _selected_learned_hotspots(
    ...
    *,
    ...
    return [
        ...
        if prediction.dirty_probability >= threshold
    ]
...
def _prediction_summary(predictions: list[HotspotPrediction], limit: int = 5) -> str:
    ...
    summary = ", ".join(
        f"F{prediction.face_id}={prediction.dirty_probability:.6f}"
        for prediction in top
    )
    return f"top dirty probabilities {summary}"
...
def _parse_probability_threshold(text: str) -> float | None:
    try:
...
def _model_status_text() -> str:
    return (
        f"{MODEL_DISPLAY_NAME}: {DEFAULT_HOTSPOT_MODEL_PATH}   "
        f"CV acc={MODEL_CV_ACCURACY:.4f}, holdout acc={MODEL_HOLDOUT_ACCURACY:.4f}, "
        f"default threshold={MODEL_DIRTY_PROBABILITY_THRESHOLD:.3f}"
    )
